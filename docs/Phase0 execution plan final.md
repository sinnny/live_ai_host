...
| **Stage 4** — Talking-head generation | ⚠ OSS half tested on RunPod, gating negative | 2026-05-12 | 2026-05-12 (OSS half) | **OSS gating result:** EchoMimic v3 Flash (the SOTA OSS audio-driven talking-head model as of 2026) fails on Korean lip-sync — `chinese-wav2vec2` audio encoder doesn't transfer to Korean phonemes. Identity preservation and visual quality at 768 are good; mouth shapes don't track Korean ㄱ/ㅋ/ㅂ/ㅍ/vowels. **Same conclusion would apply to every other OSS talking-head with public weights (Hallo3, Sonic, AniPortrait, etc.) — none trained on Korean.** Full findings at `docs/stage4_oss_findings.md`. RunPod spend: ~$0.60. Tests 2-3 (pose variants) and Preview A/B intentionally skipped — gating failure is audio→lip mapping, which is pose- and Flash/Preview-invariant. Tests 4-6 (paid commodity/premium) **still to run on fal.ai** — now more important, not less, since paid multilingual models are the only remaining path to acceptable Korean. |
