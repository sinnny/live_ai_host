...
| **Stage 2** — Identity variants | ⚠ Skipped | 2026-05-12 | 2026-05-12 | **Local PuLID generation skipped — Mac MPS doesn't support Float8_e4m3fn dtype** (TypeError on first sampling step; fundamental PyTorch 2.11 limitation). Switching to GGUF Q8_0 would test a different model than what Phase 2 production CUDA stack would use (research-vs-production trap). Decision: defer PuLID viability to Phase 1 evaluation on rented GPU (~$2-4 for 1 hour H100). For Phase 0, use **natural variants from existing reference photos** (kim_3, kim_6) as Stage 4 inputs for Tests 2 and 3 — identity preservation is automatic since they're the same person. The patched workflow JSON (`comfy_workflows/pulid_flux.json`) is preserved for the eventual Phase 1 GPU run. |
