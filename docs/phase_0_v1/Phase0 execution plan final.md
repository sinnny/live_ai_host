...
| **Stage 4** — Talking-head generation | ⚠ OSS half tested on RunPod across 2 sessions + 5 tuning levers, gating negative verified | 2026-05-12 | 2026-05-13 (OSS half) | **OSS gating result (verified):** EchoMimic v3 Flash fails on Korean lip-sync across two reference photos (hero 3/4-angle + test_2 clean front) and five orthogonal tuning levers (audio preprocessing, audio CFG bump, text CFG drop, drift negatives, image cleanup). Lip-sync failure survives all interventions — `chinese-wav2vec2` audio encoder lacks Korean phoneme representations. Identity preservation and visual quality at 768 are good. Same conclusion would apply to every other OSS talking-head with public weights (Hallo3, Sonic, AniPortrait, etc.) — none trained on Korean. Full findings at `docs/stage4_oss_findings.md`. RunPod spend: ~$0.80 across 2 sessions. Tests 2-3 (pose variants) and Preview A/B not run — pose tolerance and English wav2vec2 wouldn't fix the audio→lip mapping gap. Tests 4-6 (paid commodity/premium) **still to run on fal.ai** — now more important, not less, since paid multilingual models are the only remaining path to acceptable Korean. |
